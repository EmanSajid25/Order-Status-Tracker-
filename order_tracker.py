...
if __name__ == "__main__":
    ...
    print("👋 Welcome! Ask me about your order status by ID or full name.")
    print("Examples:\n - Where is order 1064?\n - Show me orders for Molly Bryant\nType 'exit' to quit.\n")
    ...
    while True:
        ...
        if order_id:
            print(get_order_status_by_id(order_id, orders_df))
        else:
            words = user_input.split()
            if len(words) >= 2:
                possible_name = " ".join(words[-2:])
                print(get_orders_by_customer(possible_name, orders_df))
            else:
                print("⚠️ Please enter a valid orderID or username (e.g., Nate Warren or order 1065).")
                print("you enter invalid userid and name, please enter the valid ID  and name for the order status")
